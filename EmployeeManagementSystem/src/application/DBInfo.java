...
public class DBInfo {
	...
	public static Connection getConnection() {
		...
		try {
			...
			con = (Connection) DriverManager.getConnection(url,"root",""); //// An revoir
	...
	public static int update(Employee emp) {
		...
		try {
			...
			String sql = "UPDATE employee set fname=?,lname=?,salary=?,age=?,department=?,address=?,date_of_birth=?,gender=?,education=?,contract_start=?,contract_end=?,nationality=? where id=?";
			...
			preparedStatement.setString(11, emp.getContract_end());
			preparedStatement.setString(12, emp.getNationality());
			preparedStatement.setInt(13, emp.getId());
			st = preparedStatement.executeUpdate();
